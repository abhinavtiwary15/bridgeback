...
class RemoteBackend(Backend):
    ...
    def chat(self, user_id, message, llm_backend):
        return requests.post(
            f"{API_BASE_URL}/chat",
            json={"user_id": user_id, "message": message, "backend": llm_backend},
            headers=self._headers(),
        )

    def fetch_plan(self, user_id):
        r = requests.get(
            f"{API_BASE_URL}/plan", params={"user_id": user_id}, headers=self._headers()
        )
        return r.json().get("actions", []) if r.status_code == 200 else []

    def update_action(self, user_id, action_id, status, blocker_reason=""):
        return requests.post(
            f"{API_BASE_URL}/plan/action",
            json={
                "user_id": user_id,
                "action_id": action_id,
                "status": status,
...
class LocalBackend(Backend):
    ...
    def register(self, username, password, telegram_id):
        with next(get_db()) as db:
            ...
            user = DBUser(
                username=username, hashed_password=hashed, telegram_chat_id=telegram_id
            )
            db.add(user)
            db.commit()
    ...
    def chat(self, user_id, message, llm_backend):
        ...
        result, response = process_user_message(
            engine=engine, user_id=user_id, user_text=message
        )
        structured = {}
        if response.updated_profile:
            structured = generate_structured_action_response(
                response.updated_profile, backend=engine.backend
            )
        data = {
    ...
    def update_action(self, user_id, action_id, status, blocker_reason=""):
        if status == "completed":
            ...
            block_action_task(
                action_id, blocker_reason=blocker_reason, micro_step=ms, user_id=user_id
            )
        return type("Res", (object,), {"status_code": 200})()
...
if API_BASE_URL:
    ...
    st.error(
        "No API_BASE_URL found and local dependencies missing. Check your installation."
    )
    st.stop()
...
if "messages" not in st.session_state:
    st.session_state.messages = [
        {
            "role": "assistant",
            "content": "I'm BridgeBack. Have you felt distant from anyone recently?",
        }
    ]
if "current_score" not in st.session_state:
    st.session_state.current_score = None
if "backend" not in st.session_state:
    st.session_state.backend = os.getenv("DEFAULT_LLM", "gemini")
...
if not st.session_state.access_token:
    ...
    with tab2:
        ...
        if st.button("Register"):
            handle_register(ru, rp, rchat_id)
    st.stop()
...
with col_score:
    if st.session_state.current_score is not None:
        ...
        band, css = (
            ...
            if score <= 20
            else ("Mild", "score-mild")
            if score <= 40
            else ("Moderate", "score-moderate")
            if score <= 60
            else ("Severe", "score-severe")
            if score <= 80
            else ("Acute", "score-acute")
        )
        st.markdown(
            f"**Score** <span class='score-pill {css}'>{score} — {band}</span>",
            unsafe_allow_html=True,
        )
...
with tab_chat:
    ...
    if user_input:
        ...
        with st.spinner("Thinking..."):
            res = st.session_state._backend_impl.chat(
                st.session_state.user_id, user_input, st.session_state.backend
            )
            if res.status_code == 200:
                data = res.json()
...
with tab_plan:
    ...
    if not actions:
        st.info("No actions in your plan yet.")
    else:
        for a in actions:
            st.markdown(
                f"<div class='action-card'><h4>{html.escape(a['target'])}</h4><p>Status: {a['status']}</p><p><strong>Action:</strong> {html.escape(a['action_text'])}</p></div>",
                unsafe_allow_html=True,
            )
            if a["status"] == "pending":
                if st.button("Mark Completed", key=f"comp_{a['action_id']}"):
                    st.session_state._backend_impl.update_action(
                        st.session_state.user_id, a["action_id"], "completed"
                    )
                    st.rerun()
...
with tab_progress:
    ...
    if prog:
        ...
        c1.markdown(
            f"<div class='metric-card'><div class='metric-val'>{prog.get('total_connections', 0)}</div><div class='metric-lbl'>Connections</div></div>",
            unsafe_allow_html=True,
        )
        c2.markdown(
            f"<div class='metric-card'><div class='metric-val'>{prog.get('streak', 0)}</div><div class='metric-lbl'>Wk Streak</div></div>",
            unsafe_allow_html=True,
        )
...
st.session_state.backend = st.sidebar.selectbox(
    "Backend",
    backend_opts,
    index=backend_opts.index(st.session_state.backend)
    if st.session_state.backend in backend_opts
    else 1,
)
